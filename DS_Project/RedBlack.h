...
private:
    ...
    RedBlackNode<T>* createNodeFromFile(string node,RedBlackNode<T>* prev) {
        if (node == "NULL") {
            return nil;
        }
       ...
       // file.open(repo.name + "/" + repo.currBranch + "/" +)
    ...
    void emptyTreeHelper(RedBlackNode<T>* node) {
        if (node == nullptr || node == nil)
            return;
